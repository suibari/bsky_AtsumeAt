...
import { STICKER_COLLECTION, CONFIG_COLLECTION, type Sticker, type Config, TRANSACTION_COLLECTION, type Transaction, STICKER_LIKE_COLLECTION, type StickerLike } from './schemas';
...
export async function loadStickerLikeState(agent: Agent, stickerUri: string): Promise<LikeState> {
  ...
  // 1. Get Likers DIDs
  const likerDids = await getStickerLikes(agent, stickerUri);
